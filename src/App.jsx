  ...
  useEffect(() => {
   listAll(ref(storage, 'files')).then(img=>{
    ...
    img.items.forEach(val => {
      console.log(val);
       getDownloadURL(val).then(imgUrl=> {
        console.log(imgUrl);
        // setUrl(data=>[...data,imgUrl])
       } )
    })
   })
       ...
       <div class="flex items-center border-b-2 border-[#af7eeb] py-2">
        ...
        { filtered.length > 0 ? 
          filtered.map((values, id)=>( <div key={id} className='py-2 flex items-center w-full gap-2' >
            <div className='w-full min-h-12 shadow-lg rounded-md px-4 py-6 relative' >{values.name1}

            <input id='fileImg' type="file" placeholder='nice' onChange={handleFileChange} />
            <svg  onClick={handleButton}  xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5 absolute right-2 bottom-2">
